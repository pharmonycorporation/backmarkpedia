...
export class SignalementComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.authService.getsEntite('signalements').then( (response: any) => {
           
        if(response.error){
            this.errorMessage = "Aucun signalement n'a été enregistré sur la plateforme";
        }else{
            this.filteredItems = this.signalements = response
        ...
        // console.log("les signalements : ",this.signalements)
  ...
  filterResults() {
    if (!this.searchForm.value.question) {
      this.filteredItems = [...this.signalements];
    } else {
      this.filteredItems = [];
      this.filteredItems = this.signalements.filter((commande) => {
