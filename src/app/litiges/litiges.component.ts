...
export class LitigesComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.authService.getsEntite('litiges').then( (response: any) => {
           
        if(response.error){
            this.errorMessage = "Aucun litige n'a été enregistré sur la plateforme";
        }else{
            this.filteredItems = this.litiges = response
        ...
        // console.log("les litiges : ",this.litiges)
  ...
  filterResults() {
    if (!this.searchForm.value.question) {
      this.filteredItems = [...this.litiges];
    } else {
      this.filteredItems = [];
      this.filteredItems = this.litiges.filter((commande) => {
